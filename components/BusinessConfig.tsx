...
import { BuildingStorefrontIcon, ArrowPathIcon, KeyIcon, InformationCircleIcon } from '@heroicons/react/24/outline';
...
export const BusinessConfig: React.FC<BusinessConfigProps> = ({ onConnect }) => {
  ...
  const handleAuth = () => {
    ...
    try {
      const tokenClient = initGoogleAuth(clientId, async (tokenResponse: any) => {
        if (tokenResponse && tokenResponse.access_token) {
          ...
          if (tokenResponse?.error === 'popup_closed_by_user') {
            ...
            setError("Failed to retrieve access token. Check console.");
          }
          setIsLoading(false);
        }
      });
      ...
      // @ts-ignore
      // Add prompt: 'consent' to ensure we get a fresh token if needed, mainly for debugging
      tokenClient.requestAccessToken({ prompt: 'consent' });
      
    } catch (err: any) {
      setError("Error initializing Google Auth. Did you add 'http://localhost:3000' to Authorized Origins?");
      console.error(err);
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="w-full max-w-lg mx-auto bg-white rounded-2xl shadow-xl p-8 border border-slate-100 text-center animate-fade-in-up">
      ...
      <div className="mt-8 text-xs text-slate-500 bg-slate-50 p-4 rounded-xl border border-slate-100 text-left">
        <strong className="block text-slate-700 mb-1">Setup Checklist:</strong>
        <ul className="list-disc list-inside space-y-1">
          <li>Enable <b>Google My Business API</b> in Cloud Console.</li>
          <li>Add <code>http://localhost:3000</code> to <b>Authorized JavaScript origins</b>.</li>
          <li>Add your email to <b>Test Users</b> in OAuth Consent Screen.</li>
        </ul>
      </div>
    </div>
  );
